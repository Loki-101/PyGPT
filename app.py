...
lowercase_bot_name = bot_name.lower()
...
async def generate_response(message, identifier):  # Make this function async
    try:
        response = await openai_client.chat.completions.create(  # Use await here
            model="gpt-3.5-turbo-0613", # I highly recommend leaving it on this snapshot until it's depreciated; newer models are almost un-usable due to the high level of censorship OpenAI has implemented.
            messages=[
        ...
        return str(e)
    if debug:
        print("API Response: ", response.model_dump_json())
...
async def on_message(message):
    ...
    if channel_id.lower() == "none": # If channel_id is set to none, don't check for channel ID
        if lowercase_content.startswith(f"hey {lowercase_bot_name}"):
            ...
            response = await generate_response(prompt, message.author.id)
            # Send the response
